...
```js
const getType = (value) => Object.prototype.toString.call(value).match(/(?<=\s)[a-zA-Z]+/)[0]; // 获取类型
...
input 搜索框可以使用 throttle(节流),间隔一段时间必须发请求,当 500ms 没有输入,认为输入完成,再次发请求  
只需要一次结果的适合用防抖(页面 resize 时,只需要在最后的时候进行重新渲染),
...
// 响应式, 动态改变 html 根节点的 font-size
window.onresize = throttle(() => {
  const clientWidth = document.documentElement.clientWidth || document.body.clientWidth;
  // 通过js动态改变html根节点字体大小
  const html_ = document.getElementsByTagName("html")[0];
  html_.style.fontSize = (clientWidth / 768) * 100 + "px";
}, 1000);
